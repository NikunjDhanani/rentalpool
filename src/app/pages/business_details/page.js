...
const BussinessDetails = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      bussiness_address: Yup.string().required("Business Address is Required"),
      email: Yup.string().email("Error Message").required("Email is Required"),
      flatno: Yup.string().required("FlatNo is Required"),
      pincode: Yup.number().required("Pincode is Required"),
      city: Yup.string().required("City is Required"),
      state: Yup.string().required("State is Required"),
      gstno: Yup.string()
        .matches(/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}[Z]{1}[0-9A-Z]{1}$/i, 'Invalid GST Number format')
    ...
    <main>
      ...
      >
        <Modal.Body className="text-center">
          ...
          <Button
            style={{ width: "294px", height: "56px",marginTop:'25px',padding:'10px' }}
            onClick={() => {
              setModalShow(false);
              router.push("/pages/add_products");
            }}
          >
            Add Product
          </Button>

...
const Page = () => {
  ...
  const currentItems = Array.isArray(productsData) ? productsData.slice(startIndex, endIndex) : [];
          ...
          <div className={`${styles.product_wrapper}`}>
            ...
            <div className='row'>
              {currentItems.map((product) => (
                <div key={product.id} className={`col-xl-3 col-lg-3 col-6 ${styles.product_card}`}>
                  <div className={`position-relative ${styles.product_item}`}>
                    <button className={`${styles.cancel_button}`} onClick={() => handleCancelProduct(product.id)}>
                        <Image src={cancel_btn}></Image>
                    </button>
                    <Image className={`${styles.product_image}`} src={product_image}></Image>
                    <p className={`mb-0 ${styles.product_name}`}>{product.name}</p>
                    <p className={`${styles.product_price}`}>Rs {product.price}<sub>/days</sub></p>
                    <button className={`${styles.inquiry_btn}`}>Send Inquiry</button>
                  </div>
                </div>
              ))}
            </div>
            <div className={`d-flex justify-content-center ${styles.pagination_wrap}`}>

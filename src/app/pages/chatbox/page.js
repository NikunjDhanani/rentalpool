  ...
  return (
    <div className="container-lg">
        ...
        <div className="contactus-container p-0 message-sender-side">
          ...
          <div className="message-sender-list">
            {senderMessageDetails &&
              senderMessageDetails.map((data) => {
                let key;
                key = data.whoAmI === "buyer" ? "seller" : "buyer";
                return (
                  <div
                    key={data.id}
                    className="message-sender d-flex justify-content-between align-items-center px-2 py-1"
                    onClick={() => handleSenderMessageDetails(data)}
                  >
                    <div className="d-flex align-items-center">
                      ...
                      <div className="px-2">
                        ...
                        <div className="product-name-sender">
                          {data.product.title}
                        </div>
                      </div>
                    </div>
                    <div className="text-center">
                      ...
                      {data.unread_messages !== 0 && (
                        <div>
                          <div className="total-send-message d-flex justify-content-center align-items-center">
                            {data.unread_messages > 100
                              ? "99+"
          ...
          <div className="d-flex justify-content-between align-items-center chat-box-header">
            <div>
              ...
              <span className="user-name">
                {senderUserDerails?.whoAmI === "buyer"
                  ? senderUserDerails?.seller?.name
                  : senderUserDerails?.buyer?.name}
              </span>
            </div>
            <div className="d-flex">
              <span>
                <div className="product-name">
                  {senderUserDerails?.product?.title}
                </div>
                <span className="product-amount">$ 1500/</span>
                <span className="rent-day">day</span>
              </span>
              <button
                className="btn btn-request-review"
                onClick={() => setOpenModal(true)}
                disabled={
                  !isTodayOrFuture(new Date(senderUserDerails?.last_date))
                }
              >
                {senderUserDerails?.whoAmI === "buyer"
                  ? "Give Review"
      ...
      <Modal show={openModal} onHide={handleClose}>
        ...
        <Modal.Body>
          <div className="text-center rate_your_experience">
          ...
          ></textarea>
          <div className="text-end max_2000_characters mt-2">
            Max. 2000 characters
          </div>
        </Modal.Body>
        <Modal.Footer className="justify-content-center">
          <button className="submit_review_btn" onClick={handleSendReview}>
            Submit Review
          </button>
        </Modal.Footer>
      </Modal>
